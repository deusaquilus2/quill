...
object AttachToEntity {
  ...
  def apply(f: (Ast, Ident) => Query, alias: Option[Ident] = None)(q: Ast): Ast =
    q match {
      ...
      case Map(_: GroupBy, _, _) | _: Union | _: UnionAll | _: Join | _: FlatJoin => f(q, alias.getOrElse(Ident("x", Quat.Value))) // TODO Quat. Get it from the q ast (which is the entity)
      ...
      case IsEntity(q) => f(q, alias.getOrElse(Ident("x", Quat.Value))) // TODO Quat. Get it from the q ast
