...
trait MirrorIdiomBase extends Idiom {
  ...
  implicit def queryTokenizer(implicit liftTokenizer: Tokenizer[Lift]): Tokenizer[Query] = Tokenizer[Query] {
    ...
    case Entity(name, Nil, renameable) => stmt"${tokenizeName("querySchema", renameable).token}(${s""""$name"""".token})"
  ...
  def tokenizeName(name: String, renameable: Renameable) =
    renameable match {
      ...
      case Fixed      => s"`${name}`"
      case Internal   => s"${name}/*I*/"
    }

  implicit def propertyTokenizer(implicit liftTokenizer: Tokenizer[Lift]): Tokenizer[Property] = Tokenizer[Property] {
    case Property(ExternalIdent(_), name, renameable) => stmt"${tokenizeName(name, renameable).token}"
    case Property(ref, name, renameable)              => stmt"${scopedTokenizer(ref)}.${tokenizeName(name, renameable).token}"
  }
