...
import io.getquill.util.Messages.{ TraceType, tracesEnabled, qprint }

import scala.collection.mutable
import scala.util.matching.Regex

class Interpolator(traceType: TraceType, defaultIndent: Int = 0) {
  ...
  class Traceable(sc: StringContext, elementsSeq: Seq[Any]) {
    ...
    sealed trait PrintElement
    case class Str(str: String, first: Boolean) extends PrintElement
    case class Elem(value: String) extends PrintElement
    case object Separator extends PrintElement

    private def generateStringForCommand(value: Any, indent: Int) = {
      val objectString = qprint(value).render
    ...
    private def readBuffers() = {
      ...
      val elements = elementsSeq.toList.map(qprint(_).render)
    ...
    def andLog(): Unit =
      logIfEnabled.foreach(value => println(value._1))
    ...
    def andReturn[T](command: => T) = {
      logIfEnabled() match {
        case Some((output, indent)) =>
          ...
          val result = command
          println(generateStringForCommand(result, indent))
