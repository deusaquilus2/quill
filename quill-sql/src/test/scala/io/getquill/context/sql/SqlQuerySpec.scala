...
class SqlQuerySpec extends Spec {
  ...
  //    //implicit val qs2 = schemaMeta[SimpleEnt2]("CustomEnt2", _.aa -> "field_aa")
  //
  //    val q = quote {
  ...
  "nested join - named variables - query schema" in {
    ...
    val q = quote {
      qr1.leftJoin(qr2).on { // have qr1 represent a querySchema, how does that change property propogation
        (a, b) =>
          a.i == b.i
      }.filter {
        ab =>
